...
class LogisticRegElastic():
    ...
    def __calc_betaj(self, aj, cj, lam, alp,seta, j):
        """
        ...
        if j == 0:
            lam = 0
        if cj < -lam* alp* seta[j]:
            return (cj + lam* alp* seta[j]) / (aj + lam* (1-alp)* seta[j])
        elif cj > lam* alp* seta[j]:
            return (cj - lam* alp* seta[j]) / (aj + lam* (1-alp)* seta[j])
        else:
            return 0

    def __calculate_optimal_betas(self, matrix, old_betas, lam, alp, seta,
        ...
        while beta_pct_diff > precision:
            ...
            for j in range(len(old_betas)):
                ...
                new_betas[j] = self.__calc_betaj(a_array[j], cj, lam, alp, seta,j)
    ...
    def fit(self, matrix, alp, seta, precision=.00001,
           lambda_grid = np.exp(-1 * np.linspace(1, 17, 200)), pyspark=False):
        """
        ...
        for i in range(len(lambda_grid)):
            beta_guess = self.__calculate_optimal_betas(matrix, beta_guess,
                         lambda_grid[i],alp, seta, total_trials, precision,
                         pyspark)
